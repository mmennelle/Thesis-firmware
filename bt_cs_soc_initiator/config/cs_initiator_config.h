...
// Switch to OPTIMIZED_FOR_ENERGY so the controller picks a longer connection
// interval and a higher procedure spacing. This relaxes the per-procedure
// resource budget that triggered 0x100D earlier.
#define CS_INITIATOR_DEFAULT_PROCEDURE_SCHEDULING CS_PROCEDURE_SCHEDULING_OPTIMIZED_FOR_ENERGY
...
// MEDIUM (37 channels) instead of HIGH (72) reduces the resources the
// controller has to reserve per procedure. The previous HIGH + 1:2 antennas
// combo was rejected with sl_status 0x100D
// (SL_STATUS_BT_CTRL_CONNECTION_REJECTED_DUE_TO_LIMITED_RESOURCES) from
// sl_bt_cs_set_procedure_parameters() in the cs_initiator middleware.
...
// BRD4198A initiator has 1 antenna, BRD2606A reflector has 2 -> 1:2.
// Mismatched configs trigger "Default PBR antenna configuration not supported!
// Will use the closest one and continue." and then the CS procedure dies
// with controller error 0x100d.
#define CS_INITIATOR_DEFAULT_CS_TONE_ANTENNA_CONFIG_IDX_REQ   CS_ANTENNA_CONFIG_INDEX_SINGLE_I_DUAL_R
...
// Reflector has only antenna 1 — switching across antennas would fail.
#define CS_INITIATOR_DEFAULT_CS_SYNC_ANTENNA_REQ  CS_SYNC_ANTENNA_1
