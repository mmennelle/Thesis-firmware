  ...
  switch (err_evt) {
    ...
    default:
      ...
      if (status != SL_STATUS_OK) {
        // The CONN_CLOSED event will not arrive, so the cs_initiator instance
        // and the application slot would otherwise leak. Without this cleanup
        // num_reflector_connections stays at CS_INITIATOR_MAX_CONNECTIONS
        // forever and every subsequent connection fails with 0x1c
        // (SL_STATUS_FULL) — the storm we observed.
        (void)cs_initiator_delete(conn_handle);
        delete_initiator_instance(conn_handle);
