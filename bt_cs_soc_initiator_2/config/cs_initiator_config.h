...
// <i> Default: 1866
// Raised from 1866 to 2200 to accommodate the extra (1 + mode1_size) *
// channels / main_mode_steps = 7 * 72 / 2 = 252 bytes generated when
// CS_INITIATOR_DEFAULT_CS_SUB_MODE = sl_bt_cs_mode_rtt under the HIGH
// channel map preset. Still well below the 2500-byte cap.
#ifndef CS_INITIATOR_MAX_RANGING_DATA_SIZE
#define CS_INITIATOR_MAX_RANGING_DATA_SIZE            (2200)
#endif
...
// RTT submode interleaves time-domain (round-trip) measurements with the
// PBR phase sweep. The RTL library uses RTT as a multipath sanity check
// for PBR, which substantially reduces the long-bias error PBR exhibits
// when the direct path is attenuated (e.g. tag in a pocket / behind body)
// and reflections off walls dominate. Costs ~1.5x procedure duration and
// ~250 bytes extra ranging buffer (MAX_RANGING_DATA_SIZE bumped to 2200).
#define CS_INITIATOR_DEFAULT_CS_SUB_MODE             sl_bt_cs_mode_rtt
...
// brd4198a hardware has SL_RAIL_UTIL_CS_ANTENNA_COUNT=1 (no RF switch on the
// dev kit). The controller rejects any config that needs >1 antenna path on
// this side with 0x100D. SINGLE_ONLY is the only viable config until the
// initiator board is replaced with one that has antenna-switching hardware.
#define CS_INITIATOR_DEFAULT_CS_TONE_ANTENNA_CONFIG_IDX_REQ   CS_ANTENNA_CONFIG_INDEX_SINGLE_ONLY
...
// <i> Default: 3
// Bitmask per sl_bt_api.h. For SINGLE_ONLY only 1 remote antenna is used,
// so only bit 0 must be set.
